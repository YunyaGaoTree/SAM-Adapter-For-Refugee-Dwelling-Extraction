...
def data_process_hisup(path_dataset, data_type, model_type="HiSup"):
...
def data_process_sam(path_dataset, data_type, patch_size, model_type="SAM"):
...
def upscale_img_nearest(path_in, path_out):
    ...
    resized = cv2.resize(img, dim, interpolation=cv2.INTER_NEAREST) # INTER_NEAREST INTER_LINEAR
    
    # save the upscaled image
    cv2.imwrite(path_out, resized)
    

def upscale_img_bilinear(path_in, path_out):
    ...
    resized = cv2.resize(img, dim, interpolation=cv2.INTER_LINEAR) # INTER_NEAREST INTER_LINEAR
...
def upscale_lab(path_in, path_out):
    ...
    # upsample the image
    upscale_times = 4 # percent of original size
    width = int(lab.shape[1] * upscale_times)
    height = int(lab.shape[0] * upscale_times)
    dim = (width, height)
    
    # resize image
    resized = cv2.resize(lab, dim, interpolation=cv2.INTER_NEAREST)
...
def upscale_testing_data_nearest_bilinear(path_database, dataset, upscaled_folder, data_type):
    ...
    if upscaled_folder == 'bilinear':
        resampling = Resampling.bilinear
    elif upscaled_folder == 'nearest':
    ...
    if data_type == "images":
        ...
        path_out= os.path.join(path_database, dataset, model_name, "SR", dtype, data_type)
...
def upscale_image_by_SR(path_in_img, sr_model, factor=4, patch_size=512):
